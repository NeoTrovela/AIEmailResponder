...
exports.put_user = async (req, res) => {
  ...
  try {
    ...
    let user_promise = query_database(emailresponder_db, sql_user, [data.email]);
    ...
    if(user_sql_results[0].length > 0){ // user exists
      ...
      let update_sql = `
      UPDATE users SET lastname = ?, firstname = ? WHERE email = ?;
      `;

      let update_promise = query_database(emailresponder_db, update_sql, [data.lastname, data.firstname, data.email]);
